...
async def handle_http(reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
    ...
    try:
        ...
        if path.startswith("/overlay"):
            ...
            # Serve UI with manual game selection and authentication
            try:
                # Check if user is authenticated using existing auth function
                is_authenticated = check_auth_header(req_text)
                
                # Get available games from configuration
                available_games = list(GAMES_CONFIG.keys())
                current_project = last_project or available_games[0] if available_games else "unknown"
                
                logging.info(f"[ui] Generating UI - current_project='{current_project}', available_games={available_games}, authenticated={is_authenticated}")
                
                if not is_authenticated:
                    # Check if there might be a stored token we should try
                    # Show login page
...
<head>
    ...
    <title>Socket Sentinel - Login Required</title>
    <style>
        body {{
...
<body>
    <div class="login-container">
        <div class="login-title">🔐 Socket Sentinel</div>
        <p>Authentication required to access the control panel.</p>
        
        <form class="login-form" onsubmit="handleLogin(event)">
            <div class="form-group">
                <label for="token">Access Token:</label>
                <input type="password" id="token" placeholder="Enter your authentication token" required>
            </div>
            <button type="submit" class="login-btn">🚀 Login</button>
            <div class="error-msg" id="error-msg">Invalid token. Please try again.</div>
            <div class="loading" id="loading">Checking authentication...</div>
        </form>
        
        <div style="margin-top: 20px; font-size: 0.8em; opacity: 0.7;">
            <p>If you're seeing this repeatedly, clear your browser storage and try again.</p>
        </div>
    </div>

    <script>
        // Check for stored token immediately on page load
        ...
        if (storedToken) {{
            ...
            .then(response => {{
                if (response.ok) {{
                    // Token works, load the actual UI content
                    return response.text();
                }} else {{
                    // Token is invalid, clear it and show login form
                    localStorage.removeItem('ss_token');
                    document.getElementById('loading').style.display = 'none';
                    document.getElementById('error-msg').textContent = 'Stored token expired. Please login again.';
                    document.getElementById('error-msg').style.display = 'block';
                }}
            }})
            .then(uiHtml => {{
                if (uiHtml) {{
                    // Replace the page content with the authenticated UI
                    document.documentElement.innerHTML = uiHtml;
                }}
            }})
            .catch(error => {{
        ...
        function handleLogin(event) {{
            ...
            // Test the token first before storing
            fetch('/ui', {{
                headers: {{
            ...
            .then(response => {{
                if (response.ok) {{
                    // Token is valid, store it and load UI
            ...
            .catch(error => {{
                ...
                errorMsg.style.display = 'block';
                    ...
                    # Show main UI with game selection
                    # Build game selection dropdown
                    game_options_html = ""
                    for game_key in available_games:
                        game_config = GAMES_CONFIG.get(game_key, {})
                        game_name = game_key.replace('_', ' ').title()
                        game_emojis = game_config.get("emoji", ["🎮"])
                        if isinstance(game_emojis, list):
                            game_emoji_str = " ".join(game_emojis)
                        else:
                            game_emoji_str = str(game_emojis)
                        
                        selected = "selected" if game_key == current_project else ""
                        game_options_html += f'<option value="{game_key}" {selected}>{game_emoji_str} {game_name}</option>'

                    # Get current game info
                    current_game_config = GAMES_CONFIG.get(current_project, {})
                    current_actions = current_game_config.get("actions", {})
                    current_game_emoji = current_game_config.get("emoji", ["🎮"])
                    
                    if isinstance(current_game_emoji, list):
                        game_emoji_display = " ".join(current_game_emoji)
                    else:
                        game_emoji_display = str(current_game_emoji)

                    # Build action buttons
                    action_buttons_html = ""
                    for action_key, action_emoji in current_actions.items():
                        action_count = action_counts.get((current_project, action_key), 0)
                        count_display = f" x{action_count}" if action_count > 0 else ""
                        
                        action_buttons_html += f'''
                        <button class="action-btn" onclick="triggerAction('{action_key}')">
                            <span class="action-emoji">{action_emoji}</span>
                            <span class="action-text">{action_key.replace('_', ' ').title()}{count_display}</span>
                        </button>'''

                    # Get run stats
                    current_run_num = current_run_by_project.get(current_project)
                    run_stats_html = ""
                    if current_run_num:
                        run_key = (current_project, current_run_num)
                        kills = run_stats.get(run_key, {}).get("kills", 0)
                        deaths = run_stats.get(run_key, {}).get("deaths", 0)
                        headshots = run_stats.get(run_key, {}).get("headshots", 0)
                        kd_ratio = kills / max(deaths, 1)
                        
                        run_stats_html = f'''
                        <div class="run-stats">
                            <h3>Current Run #{current_run_num}</h3>
                            <div class="stat-row">💀 Kills: {kills}</div>
                            <div class="stat-row">☠️ Deaths: {deaths}</div>
                            <div class="stat-row">🤯 Headshots: {headshots}</div>
                            <div class="stat-row">📊 K/D: {kd_ratio:.2f}</div>
                        </div>'''
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Socket Sentinel UI - {current_project.replace('_', ' ').title()}</title>
    <style>
        body {{
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
            background: linear-gradient(135deg, #1a1a2e, #16213e);
        ...
        .action-btn:active {{
            ...
            box-shadow: 0 2px 6px rgba(79, 209, 199, 0.2);
        }}
        .action-emoji {{
            font-size: 2em;
            line-height: 1;
...
<body>
    <div class="header">
        ...
        <div class="game-title">{game_emoji_display} {current_project.replace('_', ' ').title()}</div>
        <div class="game-subtitle">OBS Socket Sentinel Control Panel</div>
        <button class="refresh-btn" onclick="window.location.reload()">🔄 Refresh</button>
    </div>
    
    <div class="game-selector">
        <h3>🎮 Select Game (Manual from Lua Script)</h3>
        <select class="game-select" id="gameSelect" onchange="changeGame()">
            {game_options_html}
        </select>
        <p style="margin-top: 15px; font-size: 0.9em; opacity: 0.8;">
            This should match your selection in the OBS Lua script settings.
        </p>
    </div>
    ...
    <div class="footer">
        <p>Current game: <strong>{current_project.replace('_', ' ').title()}</strong></p>
        <p>Make sure this matches your OBS Lua script manual game selection</p>
    </div>
    ...
    <script>
        ...
        function changeGame() {{
            ...
            .then(data => {{
                if (data.success) {{
                    console.log('Game changed successfully:', data);
                    window.location.reload();
                }} else {{
                    alert('Failed to change game: ' + data.error);
                }}
            }})
            .catch(error => {{
                console.error('Error changing game:', error);
                alert('Failed to change game: ' + error.message);
            }});
        }}
        ...
        setTimeout(() => {{
            window.location.reload();
                ...
                logging.error(f"❗ [http] Failed to generate UI: {e}", exc_info=True)
                # Always show a nice error page
                body_str = f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Socket Sentinel - Error</title>
    <style>
        body {{
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
            background: linear-gradient(135deg, #1a1a2e, #16213e);
...
<body>
    <div class="error-container">
        ...
        <div class="error-title">UI Generation Error</div>
        <div class="error-message">
            The control panel encountered an error while loading. This usually indicates a temporary issue.
        </div>
        <div class="error-details">
            Error: {e}
        </div>
        <button class="retry-btn" onclick="window.location.reload()">🔄 Retry</button>
        <button class="retry-btn" onclick="window.location.href='/'">🏠 Go to Overlay</button>
    </div>
</body>
</html>"""
